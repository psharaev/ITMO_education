 ...
 */
public class PrologPrimesTester extends PrologTest {
    ...
    private final Consumer<PrologPrimesTester> check;
    public final boolean reversible;

    protected PrologPrimesTester(
            final TestCounter counter,
            final int max,
            final boolean reversible,
            final Consumer<PrologPrimesTester> check
    ) {
    ...
    void checkDivisors(final Rule rule, final boolean reversible, final Function<IntStream, IntStream> solution) {
        ...
        checkDivisors(i -> counter.test(() -> {
            ...
            if (this.reversible && reversible) {
                ...
                final List<Integer> copy = new ArrayList<>(divisors);
                random().shuffle(copy);
