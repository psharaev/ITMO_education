...
public class Md2HtmlTester {
    ...
    private final Runner runner = Runner.files("md2html.Md2Html");
    ...
    public class Checker extends BaseChecker {
        ...
        private void test(final Pair<String, String> test) {
            runner.testEquals(counter, Arrays.asList(test.getFirst().split("\n")), Arrays.asList(test.getSecond().split("\n")));
        }

        private List<String> randomMarkup() {
            return tags.values().stream().map(random::randomItem).collect(Collectors.toUnmodifiableList());
        }
