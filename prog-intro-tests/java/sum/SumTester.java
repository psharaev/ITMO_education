...
public class SumTester<T extends Number> {
    ...
    public static <T extends Number> Consumer<VariantTester<Named<SumTester<?>>>> variant(
            final Named<Supplier<SumTester<T>>> test,
            final Named<UnaryOperator<SumTester<?>>> modifier
    ) {
        return VariantTester.variant(test.getName() + modifier.getName(), modifier.getValue().apply(test.getValue().get()));
    }

    @SuppressWarnings("StaticMethodOnlyUsedInOneClass")
    public static <T extends Number> Consumer<VariantTester<Named<SumTester<?>>>> variantT(
            final Named<Supplier<SumTester<T>>> test,
            final Named<UnaryOperator<SumTester<T>>> modifier
    ) {
        return VariantTester.variant(test.getName() + modifier.getName(), modifier.getValue().apply(test.getValue().get()));
    }
