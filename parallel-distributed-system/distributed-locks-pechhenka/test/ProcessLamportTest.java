...
public class ProcessLamportTest {
    ...
    public void failsOnNoFIFO() {
        ...
        procA.onLockRequest();
        var msgsToDelay = envA.takeIncomingMessages();
        assertEquals(1, msgsToDelay.size());
        assertEquals(2, msgsToDelay.get(0).destinationPid());
        assertNull(testSystem.getSystem().getLockerPid());
        ...
        procB.onMessage(1, msgsToDelay.get(0).content());
        msgs = envB.takeIncomingMessages();
        assertEquals(1, msgs.size());
        assertEquals(1, msgs.get(0).destinationPid());
        assertEquals(2, testSystem.getSystem().getLockerPid());
        ...
        var finalMsgs = msgs;
        assertThrows(
                IllegalStateException.class, () -> procA.onMessage(2, finalMsgs.get(0).content())
        );
    }

    @SuppressWarnings("SameParameterValue")
    private void testRandomLocks(float lockProb, float unlockProb,
                                 Supplier<MessageBus> messageBusBuilder, int iterations) {
    ...
    private void testRandomLocks(float lockProb, float unlockProb, Supplier<MessageBus> messageBusBuilder) {
        testRandomLocks(lockProb, unlockProb, messageBusBuilder, 10);
    }
    ...
    public void randomLocksRandom() {
        assertThrows(IllegalStateException.class, () ->
                testRandomLocks(0.3f, 0.3f, RandomMessageBus::new, 100)
        );
    }
    ...
    public void randomLocksSmallUnlockProbRandom() {
        assertThrows(IllegalStateException.class, () ->
                testRandomLocks(0.3f, 0.05f, RandomMessageBus::new, 100)
        );
    }
}
