...
void propagate(Node *it) {
    if (it != nullptr && it->needReverse) {
        it->needReverse = false;
        swap(it->left, it->right);
        if (it->left != nullptr) it->left->needReverse ^= true;
        if (it->right != nullptr) it->right->needReverse ^= true;
    }
}
